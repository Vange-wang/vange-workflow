...
| Critical PRD/Spec/framework/workflow/architecture document | decision-readiness record, canonical source hash, sanitized review copy, Hermes `deepseek-v4-pro` report, review count `n/3`, zero unresolved serious findings | focused fresh review after serious-finding QA edits, named non-blocking Open Issues, no unrelated diff, final user confirmation before downstream execution |
...
- Critical documents: reject silent model fallback, stale reports, QA self-approval, review of a different hash, an implicit fourth review, review-count reset after edits, issue drip-feeding, taste-driven QA loops, or downstream implementation before `USER_CONFIRMATION_PENDING` is resolved.
