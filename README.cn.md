...
Vange Workflow 是面向 Codex 与 Hermes CLI 的固定角色项目协作 Skill。

它帮助项目总负责人把工作分配给已登记的专业角色任务，跟踪交接、核查证据并控制验收门禁，各专业角色负责自己的产物。它面向多任务 AI 辅助项目中的职责不清、交接遗漏、证据缺失和过早宣布完成等问题，不适合一次性问答或单个 Agent 独立完成的简单任务。

- **客户端能力：**它不是独立的通用多 Agent 平台，依赖 Codex 客户端提供所需的任务管理工具；安装 Skill 不会补齐缺失的工具。
- **授权边界：**安装不代表获得创建角色任务、fork、subagent 或后台 Agent 的授权。使用有效的已登记任务；新建必须获得用户明确授权。

**常规协作：**确认目标与权限 → 核验已登记角色 → 分发任务 → 收集产物与证据 → 返工或交接 → 全部适用门禁与用户验收通过。

**关键文档分支（需要在下游实施前触发时）：**冻结范围 → 独立获批的脱敏副本 → Hermes 审查 → 必要时由文档 QA 修复并复审 → 门禁通过与用户确认 → 下游实施。脚本输出仅为候选门禁状态；详见[审查规则与三轮上限](#7-关键文档hermesdocument-qa)。
...
这些入口仅提供预览或扫描，不代表完整多角色演示、环境配置完成或最终验收通过。首次体验不需要执行 `Apply`、调用真实审查 API、创建角色或部署生产环境。

## 文档导航

- [安装与依赖](#3-安装与依赖) · [角色注册与路由](#5-codex-固定任务与线程-id)
- [关键文档审查](#7-关键文档hermesdocument-qa) · [验收](#9-验收与完成)
- [验证](#11-验证-skill) · [发布更新](#12-发布更新)
- [运行时规则](SKILL.md) · [项目结构规范](references/project-repository-structure.md) · [任务路由契约](references/thread-routing.md)
